...
- `easystats`: es una colección de paquetes R que sirve para estadísticos y modelos en R *¡lo mejor es que dialoga con tidyverse!* [Ver más](https://easystats.github.io/easystats/)

- `gt` : nos servirá para exportar los resultados en tablas 
...
remotes::install_github("easystats/parameters") # se carga así por no estar en el CRAN... aún
```
...
### Media, media recortada y mediana
...
Otra forma de calcular el estadístico es con `chisq.test` del paquete `easystats`, donde entrega los estadísticos del estadístico
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
chisq.test(x = table(datos_proc$ocupacion,
                     datos_proc$sexo)) %>%  model_parameters(.) %>% parameters::print_html(., title = "tabla de chi2")

```

También podemos con `model_parameters` tener los resultados en una tabla, también con `print_html` podemos tener una tabla exportable. Ambas funciones son de `easystats`

```{r echo=TRUE, message=FALSE, warning=FALSE}
chisq.test(x = table(datos_proc$ocupacion,
                     datos_proc$sexo)) %>%  model_parameters(.) 
...
También con `cor.test` de `easystats` podemos calcular la correlación, para esto existen diversas formas, ahora veremos 2

En la primera se especifican las variables de los datos que se quieren extraer 

```{r casen corr-easy, echo=TRUE, results='markup'}
cor.test(datos_proc$ingreso_percapita, datos_proc$horas_mens,  
         method = c("pearson")) # se especifica el tipo de correlación

```

Mientras que en la segunda se especifica cómo formula, en ambas los resultados son los mismos, pero en esta última le diremos que queremos los parámetros en una tabla con `model_parameters` y en un formato exportable con `print_html`


```{r casen corr-easy2, echo=TRUE, results='markup'}

cor.test(formula = ~ ingreso_percapita + horas_mens,
         data = datos_proc, method = c("pearson") ) %>% 
  model_parameters(.) %>% # se pide que muestre los parámetros de la correlación
  parameters::print_html(., title = "tabla de correlación") #los transforma a una tabla exportable
...
Con `easystats` podemos generar una tabla que contenga los parámetros del estadístico

```{r echo=TRUE, message=FALSE, warning=FALSE}
aov(ingreso_percapita ~ sexo, data = datos_proc) %>% model_parameters(.) %>% # se pide que muestre los parámetros
  parameters::print_html(., title = "tabla de anova") #los transforma a una tabla exportable
```


# 10. Pruebas T-STUDENT

Para esta ocasión `easystats` tiene la función `t.test`, con la que podemos establecer los niveles de confianza con los que trabajaremos

```{r}
t.test(datos_proc$ingreso_percapita,  conf.level = 0.95) %>% model_parameters(.) %>% 
  parameters::print_html(., title = "Prueba t-student")

```


# 11. Resumen del práctico
...
- A obtener tablas de t-student

# 12. Reporte de progreso
